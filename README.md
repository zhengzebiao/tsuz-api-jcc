...
Generated by Tsu from the `python-app` template.
...
For `python-app`, it also requests `/api/profile` without a token and expects `401`, proving the auth guard responds without relying on a real access token.
